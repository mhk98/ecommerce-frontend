...
const Register = () => {
    ...
    const handleRegister = async(data) =>{
      ...
      let res = await postRequest(`customer/register`, data);
      if(res?.success){
  ...
  return (
    <div className="mt-4 pb-6">
      <form className="space-y-7" onSubmit={handleSubmit(handleRegister)}>
        ...
        <button
          type="submit"
          className="flex justify-center bg-primary py-3 rounded-full w-full text-white font-semibold tracking-wider"
        >
          Create account
        </button>
      </form>
